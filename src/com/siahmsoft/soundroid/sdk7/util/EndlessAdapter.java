 ...
 * serve as both a placeholder while more data is being appended, and then later
 * an actual row in the result set. You might accomplish this via two widgets in
 * a FrameLayout, only one of which is visible (e.g., an ImageView doing a
 * rotation animation while loading new data, then your regular row).
 * 
 * Subclasses will be handed that row View back on rebindPendingView() so they
 * can flip back to normal mode.
 * 
...
abstract public class EndlessAdapter extends AdapterWrapper {
	...
	abstract protected void hideTrobber(int position, View row);
	...
	class AppendTask extends AsyncTask<Void, Void, Void> {
		...
		protected void onPostExecute(Void unused) {
			/*
			 * appendCachedData(); rebindPendingView(pendingPosition,
			 * pendingView); pendingView=null; pendingPosition=-1;
			 */
			if (keepOnAppending.get()) {
				appendCachedData();
				...
				hideTrobber(pendingPosition, pendingView);
			}
		}
	}
}